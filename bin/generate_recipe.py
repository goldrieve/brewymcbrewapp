...
def generate_recipe(model, style, abv, og, fg, batch_size, 
    ...
    # Extract predictions
    grain_indices = torch.argmax(outputs['grain_logits'], dim=1).item()
    grain_weights = outputs['grain_weights'][0].tolist()
    
    hop_indices = torch.argmax(outputs['hop_logits'], dim=1).item()
    ...
    for i, (score, idx) in enumerate(zip(top_grains.values, top_grains.indices)):
        if i < len(grain_weights) and grain_weights[i] > 0:
            ...
            if weight < 50:
                recipe_text += f"  - {weight:.2f}kg {grain_name}\n"
            else:
...
def main():
    ...
    parser.add_argument('--model', type=str, default='data/recipe_template_model.pt', help='Model file')
